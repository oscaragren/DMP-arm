...
def _plot_dmp(angles: np.ndarray, q_gen: np.ndarray, trial_dir: Path, meta: dict) -> None:
    plot_dmp_single(
        q_demo=angles,
        q_gen=q_gen,
        meta=meta,
        title_suffix="dmp",
        out_path=trial_dir / "dmp.png"
    )
...
def main():
    ...
    _save_angles(angles, trial_dir, t, dt)
    _save_dmp_model(dmp_model, trial_dir)
    _save_dmp_rollout(q_gen, trial_dir, t, dt)
    
    # 8) Plot the angles and DMP trajectory
    _plot_3d_trajectory(seq, t, meta, trial_dir / "sequence.png")
    _plot_angles(angles, trial_dir, t, meta)
    _plot_dmp(angles, q_gen, trial_dir, meta)
